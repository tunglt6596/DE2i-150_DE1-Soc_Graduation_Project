...
// DMA_2 is at offset 0x30 in Qsys
#define DMA_READ_ADD_OFFSET				12+0
...
static int char_device_open(struct inode *inodep, struct file *filep) {
	...
	__write( DMA_base_ptr, DMA_READ_ADD_OFFSET + 1, __pa(copy_from_1) );
	__write( DMA_base_ptr, DMA_READ_ADD_OFFSET, __pa(copy_from_1) );
